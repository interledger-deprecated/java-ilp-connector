...
/**
 * A centralized service that manages all of a Connector's {@link LedgerPlugin} instances for a given connector
 * Interledger address. This service manages functionality relating to plugin startup, graceful shutdown, and more.
 *
 * Note that event listeners are not managed by this manager, but are instead registered by external callers, and react
 * to error by potentially removing themselves as listeners.
 */
public interface LedgerPluginManager {
   ...
   *
   * @param ledgerPluginConfig A {@link LedgerPluginConfig} with configuration information to apply to the plugin.
   ...
   * Add a new plugin to this manager for the given {@code ledgerPrefix} and {@code tradingPair}.
   *
   * @param ledgerPrefix A {@link InterledgerAddress} corresponding to the supplied ledger plugin to remove.
   */
   ...
   *
   * This method acts as a kind of precondition check for the presence of a ledger plugin because, if this method fails,
   * it's probable that the plugin being accessed might actually be the plugin we would send an ILP rejection message
   * back upon, which would be impossible if the plugin were missing. Thus, there is likely no recovery path for this
   * type of exception exists except for the connector to perhaps retry the operation in the future.
   *
