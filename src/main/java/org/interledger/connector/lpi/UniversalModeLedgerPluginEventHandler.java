...
/**
 * An implementation of {@link LedgerPluginEventHandler} that handles events from Ledger plugins
 * running in a Connector, using ILP Universal-mode behavior.
 *
 * @see "https://github.com/interledger/rfcs/blob/master/0001-interledger-architecture/
 *     0001-interledger-architecture.md"
...
public class UniversalModeLedgerPluginEventHandler extends
     ...
     *
     * In Universal-mode, this connector is incentivized to pass this fulfillment back to the ledger
     * that prepared the corresponding source transfer so that this connector can capture any funds
     * waiting for it.
     *
     * In Atomic-mode, this is less important, because the ledger underlying the source transfer
     * will not fulfill until it consults its notary. However, passing the fulfillment back to the
     * source ledger might still be desirable in order to prompt the source ledger to consult with
     * its notary.
     *
