...
/**
 * An abstract implementation of {@link LedgerPluginEventHandler} that handles events from Ledger
 * plugins running in a Connector.
 */
public abstract class AbstractLedgerPluginEventHandler implements LedgerPluginEventHandler {
    ...
    /**
     * Given a source transfer and information about the "next hop" in an Interledger payment chain,
     * construct a new {@link Transfer} that can be used to complete this Interledger payment.
     */
    ...
    /**
     * Called when an incoming transfer has been fulfilled on the underlying ledger. For standard
     * behavior, this is a no-op because in general, this connector was the one that would have
     * passed the fulfillment to that ledger plugin.
     *
    ...
    /**
     * Called when an incoming transfer has expired on the underlying ledger. This event may be
     * emitted by the ledger-plugin, but it might also be emitted by an Atomic-mode validator.
     *
     * For this implementation, this is currently a no-op. However, if this is occurring in a
     * Universal-Mode Connector, it may be desirable to track this, because it _may_ have occurred
     * whilst an outgoing transfer was waiting to be prepared, and/or might have been fulfilled. In
     * that instance, this connector likely would lose money.
     *
     * In Atomic-Mode usage, this callback merely indicates that the source-transfer ledger
     * cancelled the transaction. However, this is likely still just a no-op because either no
     * destination transfer has yet been prepared, in which case nothing need happen. Or, the
     * destination ledger will consult the same Atomic-Mode notary and likewise reject the
     * destination transfer, meaning no action is necessary here.
     *
     * One optimization that could be made is for the Connector to respond to this method by
     * checking to see if any incoming_transfer events have yet to be processed, and preemptively
     * _not_ prepare on the destination ledger, but this is likely an optimization that would
     * slow-down the happy path due to checking in the prepare handler, so the optimization may not
     * be worth it.
     *
    ...
    /**
     * Called when an incoming transfer that was rejected by this connector has completed its
     * rejection.
     *
    ...
    /**
     * Called when an outgoing transfer has been prepared on the underlying, destination ledger. For
     * standard behavior, this is a no-op because in general, this connector was the one that would
     * have prepared the transfer in the first place, so other than logging the notification,
    ...
    /**
     * This is event is emitted if an outgoing transfer prepared by this connector is cancelled by
     * the destination ledger (i.e., it timed out).
     *
    ...
    /**
     * This is event is emitted if an outgoing transfer prepared by this connector is rejected by
...
//  }
    ...
    /**
     * Given a prepared source transfer, and a ready to transmit outgoing transfer, prepare the
     * destination transfer on the appropriate ledger plugin.
     *
     * If the destination transfer cannot be prepared, for whatever reason, then reject the incoming
     * source transfer with an appropriate ILP error code.
     */
    @VisibleForTesting
    ...
    /**
     * Map an instance of {@link LedgerPluginException} to a corresponding {@link
     * InterledgerProtocolError} for sending back to a source ledger.
     */
    @VisibleForTesting
     ...
     * @param rejectionReason             A {@link InterledgerProtocolError} containing information
     *                                    from the destination ledger about why that destination
